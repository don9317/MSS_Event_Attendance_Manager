...
function openWalkUp(){
  if(!appSettings.methodManual){alert('Walk-up entry is disabled in Session Builder. Enable Manual/walk-in entry fallback first.');return;}
  ['walkUpName','walkUpGrade','walkUpAge','walkUpParent','walkUpPhone','walkUpEmail','walkUpTeam'].forEach(id=>{if($(id))$(id).value='';});
  if($('walkUpPaid'))$('walkUpPaid').checked=false;
  const source=$('walkUpSource');if(source){source.innerHTML=`<option value="Walk-Up">Walk-Up</option><option value="MSS">MSS</option><option value="${(appSettings.secondarySourceName||'Other Source').replace(/"/g,'&quot;')}">${appSettings.secondarySourceName||'Other Source'}</option>`;source.value='Walk-Up';}
  $('walkUpSessionLabel').textContent=`${selectedDay()} • ${selectedSession()}`;
  const areaField=$('walkUpAreaField'),area=$('walkUpArea');
  areaField.classList.toggle('hidden',!appSettings.ruleAssignArea);
  area.innerHTML='<option value="">Select area</option>'+((appSettings.areas||[]).map(a=>`<option value="${a.replace(/"/g,'&quot;')}">${a}</option>`).join(''));
  $('walkUpMsg').classList.add('hidden');$('walkUpMsg').textContent='';
  $('walkUpModal').classList.add('show');
  setTimeout(()=>$('walkUpName')?.focus(),50);
}
function closeWalkUp(){$('walkUpModal').classList.remove('show');}
function saveWalkUp(){
  const name=clean($('walkUpName').value),grade=clean($('walkUpGrade').value),age=clean($('walkUpAge').value),phone=clean($('walkUpPhone').value),email=clean($('walkUpEmail').value),area=clean($('walkUpArea')?.value);
  const msg=$('walkUpMsg');msg.classList.add('hidden');
  if(!name){msg.textContent='Camper name is required.';msg.classList.remove('hidden');$('walkUpName').focus();return;}
  if(appSettings.ruleRequireGrade&&!grade&&!age){msg.textContent='Enter either grade or age before check-in.';msg.classList.remove('hidden');return;}
  if(appSettings.ruleAssignArea&&!area){msg.textContent='Select a court or area before check-in.';msg.classList.remove('hidden');return;}
  const phoneDigits=normPhone(phone);
  const duplicate=people.find(p=>low(p.name)===low(name)&&(phoneDigits?normPhone(p.phone)===phoneDigits:true));
  if(duplicate){
    if(attendanceFor(duplicate)){msg.textContent=`${duplicate.name} is already checked into this session.`;msg.classList.remove('hidden');return;}
    if(!confirm(`${duplicate.name} already exists in the participant list. Check in the existing participant instead of creating a duplicate?`))return;
    if(appSettings.ruleAssignArea){duplicate.areaAssignments=duplicate.areaAssignments||{};duplicate.areaAssignments[slotKey()]=area;}
    closeWalkUp();toggleCheck(duplicate.id);return;
  }
  const source=clean($('walkUpSource').value)||'Walk-Up';
  const type=source==='MSS'?'Public':(source===appSettings.secondarySourceName?'Swarm':'Walk-Up');
  const stamp=Date.now().toString(36)+Math.random().toString(36).slice(2,7);
  const p={id:`walkup-${stamp}`,name,parent:clean($('walkUpParent').value),email,phone,team:clean($('walkUpTeam').value),grade,age,session:selectedSession(),memberId:'',qr:'',source,type,waiver:false,homework:false,walkUp:true,walkUpPaid:!!$('walkUpPaid').checked,createdAt:new Date().toISOString(),areaAssignments:{}};
  if(area)p.areaAssignments[slotKey()]=area;
  people.push(p);
  const rec={key:recordKey(p.id),participantId:p.id,date:selectedDay(),session:selectedSession(),status:'present',arrival:appSettings.ruleArrival?nowTime():'',homework:false,area,source:p.source,type:p.type,team:p.team,name:p.name,walkUp:true,paymentHandled:p.walkUpPaid,createdAt:new Date().toISOString()};
  attendanceRecords.push(rec);history=attendanceRecords;save();closeWalkUp();renderAll();
  showScanToast('WALK-UP CHECKED IN',`${p.name}<br>${selectedSession()}${area?'<br>'+area:''}`,'green');
}
