...
function renderKpis(){const cohort=visiblePeople();const checked=cohort.filter(p=>isChecked(p)).length;$('kRegistered').textContent=cohort.length;$('kChecked').textContent=checked;$('kWaiver').textContent=cohort.filter(p=>needsWaiver(p)).length;$('kHomework').textContent=cohort.filter(p=>p.homework).length;}
